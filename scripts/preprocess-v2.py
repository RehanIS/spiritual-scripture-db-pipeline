...
def chunk_paragraphs(paragraphs: list[str], max_words: int) -> list[str]:
    chunks = []
    current_chunk = []
    ...
    for para in paragraphs:
        ...
        if wc > max_words:
            ...
            for s in sentences:
                s_wc = len(s.split())
                if current_word_count + s_wc > max_words:
...
def main():
    ...
    paragraphs = normalize_text(raw_text)
    chunks = chunk_paragraphs(paragraphs, MAX_WORDS_PER_CHUNK)
